...
class TestEmptyStrings:
    def test_empty_string_text_mode(self, fake_font: FakeTTFont):
        s = ANSIString("")
        svg = s.to_svg(fake_font, font_size_px=16)  # type: ignore
        assert "<svg " in svg
        assert "</svg>" in svg

    def test_empty_string_path_mode(self, fake_font: FakeTTFont):
        s = ANSIString("")
        svg = s.to_svg(fake_font, font_size_px=16, convert_text_to_path=True)  # type: ignore
        assert "<svg " in svg
        assert "</svg>" in svg
...
class TestMultiLine:
    ...
    def test_multiline_height_grows(self, fake_font: FakeTTFont):
        ...
        s2 = ANSIString("Line1\nLine2").fg_24b(0, 0, 0)
        svg1 = s1.to_svg(fake_font, font_size_px=16)  # type: ignore
        svg2 = s2.to_svg(fake_font, font_size_px=16)  # type: ignore
...
class TestFonttoolsGuard:
    def test_raises_when_fonttools_disabled(self):
        ...
        with pytest.raises(ImportError):
            s.to_svg(None, font_size_px=16)  # type: ignore
...
class TestModeParity:
    def test_both_modes_contain_svg_tags(self, fake_font: FakeTTFont):
        ...
        for mode in (False, True):
            svg = s.to_svg(fake_font, font_size_px=16, convert_text_to_path=mode)  # type: ignore
            assert "<svg " in svg
            assert "</svg>" in svg
            assert 'fill="rgb(255, 0, 0)"' in svg
